...
CREATE TABLE schema_versions (
  ...
  change_diff    TEXT,               -- json-diff output vs prior version, for audit display
  applied_by     TEXT,
  created_at     TEXT NOT NULL DEFAULT (datetime('now'))
);
...
- Fully typed for everything structural: `collections`, `schema_versions`, `media`, `audit_log`, fixed `content` columns (`collection_id`, `status`, timestamps, joins, pagination).
...
GET    /audit?target_type=&target_id=            schema:admin
...
- Static paths (auth, media, audit, param shapes) generated by Hono's OpenAPI tooling from route definitions.
