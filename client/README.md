...
Typed API client for pouch, generated from the live OpenAPI spec.

## Generate a client

pouch exposes a live OpenAPI 3.1 spec at `/openapi.json`. You can generate a typed client in any project using `openapi-typescript` + `openapi-fetch`:
...
npx openapi-typescript https://your-cms.workers.dev/openapi.json \
  --header "Authorization: Bearer $POUCH_TOKEN" \
...
const client = createClient<paths>({
  ...
  headers: { Authorization: `Bearer ${POUCH_TOKEN}` },
});
...
Because the pouch assembler expands `/collections/{slug}/content` into concrete paths per collection (`/collections/faq/content`, `/collections/best_deals/content`, etc.), the generated client uses those concrete paths directly. Query filters are typed per collection schema, e.g. `?scope=general` or `?price[gt]=20000`.
...
Refresh the local `openapi.json` snapshot from the running pouch dev server, then regenerate TypeScript types:

```bash
POUCH_TOKEN=<your-token> pnpm typegen
```

The script fetches the live spec automatically. To update the snapshot manually:

```bash
curl -H "Authorization: Bearer $POUCH_TOKEN" http://localhost:3200/openapi.json > ./openapi.json
pnpm typegen
```
...
```bash
POUCH_TOKEN=<your-token> pnpm demo
```
